...
/**
 * Convert a window of real-time HR samples into an HSI JSON payload.
 *
 * Uses parallel arrays for a clean C ABI. Each element at index i of
 * bpm_values and timestamp_ms_values forms one HR sample.
 *
 * @param session_id         Unique session identifier (null-terminated).
 * @param device_id          Device instance identifier (null-terminated).
 * @param timezone           User's timezone, e.g., "America/New_York" (null-terminated).
 * @param start_epoch_ms     Window start as Unix epoch milliseconds.
 * @param end_epoch_ms       Window end as Unix epoch milliseconds.
 * @param bpm_values         Array of heart rate values in BPM (length = sample_count).
 * @param timestamp_ms_values Array of Unix epoch ms timestamps (length = sample_count).
 * @param sample_count       Number of samples in the parallel arrays.
 *
 * @return Newly allocated HSI JSON string.
 *         Returns NULL on error; call flux_last_error() for details.
 *         Caller must free with flux_free_string().
 */
char* flux_hr_window_to_hsi(
    const char* session_id,
    const char* device_id,
    const char* timezone,
    int64_t start_epoch_ms,
    int64_t end_epoch_ms,
    const float* bpm_values,
    const int64_t* timestamp_ms_values,
